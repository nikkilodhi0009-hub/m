    ...
    fun installFrameworkHooks() {
        ...
        targets.forEach targetLoop@{ target ->
            target.classNames.forEach { className ->
                target.methodNames.forEach methodLoop@{ methodName ->
                    val registrationId = "${className}:${methodName}"
                    if (installedTargets.contains(registrationId)) {
                        return@methodLoop
                    }
                    registerJavaHook(className, methodName, object : HookCallback {
                        override fun before(args: Array<Any?>) = true
